@extends('layouts.app')
@section('title', '#' . $tag->name . ' — Blog — Amsaz Cosmetics')
...
<section class="category-hero">
    <div class="container">
        <h1 class="category-hero__title">
            Posts tagged <span style="color:#c9a96e;">#{{ $tag->name }}</span>
        </h1>
        <p class="category-hero__desc">{{ $posts->total() }} article(s) with this tag</p>
    </div>
</section>
    ...
    <div>
        @if($posts->isEmpty())
            <div style="text-align:center;padding:4rem;color:#9ca3af;">
                <p>No posts with this tag. <a href="{{ route('blog.index') }}" style="color:#c9a96e;">View all posts →</a></p>
            </div>
        @else
            <div class="blog-grid">
